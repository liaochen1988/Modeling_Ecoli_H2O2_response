...
for i=1:length(Gor)
    for j=1:length(FC)
        ...
        IC(2) = glucose;
        [t,x] = ode15s(@Kinetic_model2,[-10000.0*3600.0 0.0],IC,options,arcA_mutant,fnr_mutant,a,true,GSSG,Gor(i),kd_nadph(i));
        IC = x(end,:);
        [t,x] = ode15s(@Kinetic_model2,[0.0 60.0],IC,options,arcA_mutant,fnr_mutant,a,true,GSSG*FC(j),Gor(i),kd_nadph(i));
