...
%% Run NADPH response
%Trx_ox = 0; % M
Trx_ox = [1e-5, 5e-5, 1e-4, 5e-4, 1e-3, 5e-3, 1e-2, 5e-2, 1e-1];     % M
% TrxR = 0.42 * 1e-6;         % M
TrxR = [5e-7, 1e-6, 5e-6, 1e-5];
cls = jet(length(Trx_ox));

figure();

for i=1:length(TrxR)
    for j=1:length(Trx_ox)
        [i,j]
        [t,x] = ode15s(@Kinetic_model,[0.0 60.0],IC,options,arcA_mutant,fnr_mutant,a,true,Trx_ox(j),TrxR(i));
