function dx = Kinetic_model(t,x,arcA_mutant,fnr_mutant,a,called_by_ode15s,kd_nadph,TrxR,H2O2,k_Trxox_H2O2)
...
NADPH_uc = NADPH * 564 * 1e-6; % M
v_TrxR = 41.25 * NADPH_uc * Trx_ox * TrxR ...
        / (4.6e-6 * NADPH_uc + 1.7e-6 * Trx_ox + NADPH_uc * Trx_ox); % M/s

Bcp = 0.478 * 1e-6; % M
v_Bcp = 1.24 * H2O2 * Trx_red * Bcp / (5e-4 * H2O2 + 7.61e-5 * Trx_red + H2O2 * Trx_red);
Tpx = 4.88 * 1e-6; % M
v_Tpx = 76 * H2O2 * Trx_red * Tpx / (2.55e-5 * H2O2 + 1.73e-3 * Trx_red + H2O2 * Trx_red);
Msr = 2.35e-6; % M
Met_ox = 3e-6; % M
v_Msr = 3.7 * Met_ox * Trx_red * Msr / (1e-5 * Met_ox + 1.9e-3 * Trx_red + Met_ox * Trx_red);
GSSG = 0.00237; % M
v_GSSG = 100 * GSSG * Trx_red;
v_H2O2 = k_Trxox_H2O2 * H2O2 * Trx_red;

% Compared to KatG
% 1.63e4 H2O2 KatG / (3.9e-3 + H2O2) ~= 4.18e6 * H2O2 * KatG
...
if called_by_ode15s,
    ...
    % NADPH conc.
    dx(37,1) = v_G6PDH + v_PGDH + v_ICDH + v_Mez - v_TrxR * 1e6 / 564 - kd_nadph * NADPH;
    % Oxidized Trx conc.
    dx(38,1) = v_Tpx + v_Bcp + v_Msr + v_GSSG + v_H2O2 - v_TrxR;
    % Reduced Trx conc.
    dx(39,1) = - v_Tpx - v_Bcp - v_Msr - v_GSSG - v_H2O2 + v_TrxR;
