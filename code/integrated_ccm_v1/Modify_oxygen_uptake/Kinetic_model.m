function dx = Kinetic_model(t,x,arcA_mutant,fnr_mutant,a,called_by_ode15s,new_qOCR,c_metE)
...
% MetE concentration
MetE   = (total_aa * (1 - phi_Q) - l_rib * RIB) * alpha_metE / l_metE;
assert (MetE >= 0.0);

% Met synthesis
v_Bio_MET = k_ATP * v_ATP * MetE * c_metE;
...
if called_by_ode15s,
    ...
    % ATP
    dx(37,1) = v_ATP - (v_Bio_AA * 6 + v_Bio_RIB * 4566 * 10) / uc_gDCW_L - mu * ATP;
    % Methionine
    dx(38,1) = v_Bio_MET - v_Bio_AA * f_met  - mu * MET;
    % Ribosome
    dx(39,1) = v_Bio_RIB - mu * RIB;
    % ppGpp
    dx(40,1) = v_Bio_ppGpp - v_Deg_ppGpp - mu * ppGpp;
    
    if (new_qOCR)
        % O2
        dx(41,1) = 1.64 * (a*0.214375/100.0 - O2) - (v_Cyo + v_Cyd) / 2 * 1e-3 * uc_gDCW_L  - mu * O2;
    end
