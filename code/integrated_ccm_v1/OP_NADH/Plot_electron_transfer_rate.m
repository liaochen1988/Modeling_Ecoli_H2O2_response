...
for i=1:length(GLC_ex_input)
    ...
    for j=1:length(a)   
        ...
        electron_transfer_rate(i,j) = Kinetic_model(0,x(end,:),arcA_mutant,fnr_mutant,a(j),called_by_ode15s,integrated_model,c_met_e);
...
for i=1:length(GLC_ex_input)
    plot(a, electron_transfer_rate(i,:), 'k-o', 'Color', cc(i,:));
end
