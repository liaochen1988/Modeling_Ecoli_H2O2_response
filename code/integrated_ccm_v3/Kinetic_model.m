function dx = Kinetic_model(t,x,arcA_mutant,fnr_mutant,katE_mutant,katG_mutant,a,V_env,called_by_ode15s,const_ext_vars)
...
% ICDH
v_ICDH = ((1.0+TF_Cra)*(1.0-TF_ArcA)) * ICDH_Vmax * (ICT/ICDH_Kict) * (1+ICT/ICDH_Kict)^(ICDH_n-1) / ((1+ICT/ICDH_Kict)^ICDH_n+ICDH_L*(1+PEP/ICDH_Kpep)^ICDH_n) * NADP / (NADP + KmNADP_ICDH) * ICDH_act;
v_ICDH = ((1.0+TF_Cra)*(1.0-TF_ArcA)) * ICDH_Vmax * (ICT/ICDH_Kict) * (1+ICT/ICDH_Kict)^(ICDH_n-1) / ((1+ICT/ICDH_Kict)^ICDH_n+ICDH_L*(1+PEP/ICDH_Kpep)^ICDH_n) * ICDH_act;
v_ICDH_act = 0.075 * (ICDH_act - 1 / (1  + (H2O2 / 3e-5)));
...
% H2O2 transport (M/s)
v_H2O2_trans = (H2O2_ex - H2O2) * PA / V_cell;
...
% KatG
v_Bio_katG = alpha_max_katG * frac_oxyr;
% AhpCF
v_Bio_ahpCF = alpha_max_ahpCF * frac_oxyr;
% Grx
v_Bio_grx = alpha_max_grx * frac_oxyr;
% Gor
v_Bio_gor = 5.00e-9 * frac_oxyr * 0.7;
...
% KatG
v_deg_h2o2_katG = kcat1_katG * KatG * H2O2 / (Km1_h2o2_katG + H2O2);
v_inv_h2o2_katG = kcat2_katG * KatG * H2O2 / (Km2_h2o2_katG + H2O2);
% AhpCF
v_deg_h2o2_ahpCF = kcat1_ahpCF * AhpCF * H2O2 / (Km1_h2o2_ahpCF + H2O2) * NADH / (Km1_nadh_ahpCF + NADH);
v_inv_h2o2_ahpCF = kcat2_ahpCF * AhpCF * H2O2 / (Km2_h2o2_ahpCF + H2O2);
...
% Thiol proteome
v_PSH_oxi = 5e5 * H2O2 * PSH * 0;
v_PSH_red = 2.1e6 * PSS * Trx_red;
% MetE
v_MetE_oxi = 0.230 * MetE_SH * GSSG;
v_MetE_red = 0.165 * MetE_SSG * GSH;
v_MetE_red2 = 5.16e6 * MetE_SSG * GSH ^ 2  / (MetE_SSG * 1e-6 + 1.451 * GSSG + 10.098 * GSH ^ 2) * Grx;
% OxyR
v_OxyR_oxi = kon_OxyR_h2o2 * H2O2 ^ n_OxyR / (Km_OxyR ^ n_OxyR + H2O2 ^ n_OxyR) * OxyR_red;
v_OxyR_oxi2 = kon_OxyR_grx * OxyR_red * GSSG ^ 4 * Grx;
v_OxyR_red2 = koff_OxyR_grx * OxyR_ox * GSH ^ 8 * Grx;
% GSH
v_GSH_oxi =5e4*H2O2*GSH^2;%15.2 * H2O2 * GSH / (1 + 10^(-7) / 10^(-8.44));
v_GSH_red = 1e2 * GSSG * Trx_red;
% Gor-mediated reduction (M/s)
v_GSH_red2 = (267 * GSSG  + 6.55e5 * GSSG ^ 2) * (NADPH * uc_gDCW_L * 1e-6) * Gor / ...
             (2.22e-5 * GSSG + 9.7e-5 * (NADPH * uc_gDCW_L * 1e-6) + GSSG * (NADPH * uc_gDCW_L * 1e-6) + 0.022 * GSSG ^ 2 + 3.9e3 * GSSG ^ 2 * (NADPH * uc_gDCW_L * 1e-6));
% Thioredoxin (M/s)
...
if called_by_ode15s,
    ...
    %dx(38,1) = v_G6PDH + v_PGDH + v_ICDH + v_Mez - (v_GSH_red2 + v_Trx_red) * 1e6 / uc_gDCW_L - v_NADPH_drain;    % NADPH
    dx(38,1) = v_G6PDH + v_PGDH + v_ICDH + v_Mez - 150 * H2O2 - v_NADPH_drain;    % NADPH
    dx(60,1) = - dx(38,1) * 0;                                                                                    % NADP
    % Methionine
    ...
    % Thiol proteome
    dx(42,1) = - 1 * (PSH - 3) - v_PSH_oxi + v_PSH_red;                                                                           % PSH
    %dx(43,1) = - dx(42,1);                                                                                       % PSS
    % MetE
    ...
    % H2O2
    dx(47,1) = - v_H2O2_trans_ex;                                                                                 % H2O2_ex
    dx(48,1) = v_H2O2_metab_prod + v_H2O2_trans - v_deg_h2o2_katG - v_deg_h2o2_ahpCF ...
               - v_inv_h2o2_katG - v_inv_h2o2_ahpCF ...
               - n_oxyr * v_OxyR_oxi - v_PSH_oxi*0 - v_Met_oxi - v_GSH_oxi;                                 % H2O2
    % OxyR
    dx(49,1) = v_OxyR_oxi + v_OxyR_oxi2 - v_OxyR_red2;                                                            % OxyR_ox
    dx(50,1) = - dx(49,1);                                                                                        % OxyR_red
    % OxyR-dependent gene upregulation
    dx(51,1) = 0;                                                                                                 % KatE
    dx(52,1) = v_Bio_katG - v_inv_h2o2_katG - (mu + log(2)/Protein_half_life) * KatG;                             % KatG
    dx(53,1) = v_Bio_ahpCF - v_inv_h2o2_ahpCF - (mu + log(2)/Protein_half_life) * AhpCF;                          % AhpCF
    dx(54,1) = v_Bio_grx - (mu + log(2)/Protein_half_life) * Grx;                                                 % Grx
    dx(55,1) = v_Bio_gor - (mu + log(2)/Protein_half_life) * Gor;                                                 % Gor
%     % Glutathione
%     dx(56,1) = - 0*v_GSH_oxi * 2 + 0*v_GSH_red * 2 + 0*v_GSH_red2 * 2 ...
%                + v_MetE_oxi - v_MetE_red - v_MetE_red2 ...
%                + v_OxyR_oxi2 * 8 - v_OxyR_red2 * 8;                                                               % GSH                                  
%     dx(57,1) = 0*v_GSH_oxi - 0*v_GSH_red - 0*v_GSH_red2 ...
%                - v_MetE_oxi + v_MetE_red + v_MetE_red2 ...
%                - v_OxyR_oxi2 * 4 + v_OxyR_red2 * 4;                                                               % GSSG                                                                                  % GSSG
    % Glutathione
    dx(56,1) = - 1 * (GSH - 0.0166) - v_GSH_oxi * 2 + v_GSH_red * 2 + v_GSH_red2 * 2;                                              % GSH                                  
    dx(57,1) = - 1 * (GSSG - 0.00237) + v_GSH_oxi - v_GSH_red - v_GSH_red2;                             % GSSG                                                                                  % GSSG
...
%     dx(59,1) = v_Trx_red - v_Met_red - v_PSH_red;                                                                 % Trx_red
    ...
    dx = [mu, v_H2O2_metab_prod, frac_oxyr, v_ICDH, v_CS, v_Icl, v_GSH_red2];
