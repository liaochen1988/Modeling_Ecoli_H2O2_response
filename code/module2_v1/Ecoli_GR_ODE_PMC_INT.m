function [dxdt,growthRate,kelong,fracCharge,fracRactive,RCF] = Ecoli_GR_ODE_PMC_INT(t,x,nutr,cm,hp)
...
RCF     = hp.thetaPpGppR./(hp.thetaPpGppR+ppGpp);
...
dxdt(1) = hp.tauA*(JsAa      -kelongTot*hp.fAa   -growthRate*AminoAcid);
dxdt(2) = hp.tauR*(JsR       -JdR                -growthRate*Ribosome);
dxdt(3) = hp.tauG*(JsPpGpp   -JdPpGpp            -growthRate*ppGpp);
dxdt(4) = growthRate;
dxdt    = dxdt';
