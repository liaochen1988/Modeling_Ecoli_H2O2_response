function [kcat,growthRate,AminoAcid,Ribosome,ppGpp] = run_nutrient_limitation(hp)

%   hp: host cell parameters

kcat                = 10.^[-4:0.05:4]/3600;  % turn over rate of metE
AminoAcid           = zeros(length(kcat),1);
...
x0 = [10,10,10];
tic;
[~,x,te] = ode15s(@Ecoli_GR_ODE_PMC,tspan,x0,options_ode15s_PMC,kcat(end),hp);
if (isempty(te))
    x0 = x(end,:);
else
...
for i=length(kcat):-1:1
    tic;
    [~,x,te] = ode15s(@Ecoli_GR_ODE_PMC,tspan,x0,options_ode15s_PMC,kcat(i),hp);
    if (~isempty(te))
        error('Error: Oscillation Detected for the current parameter set!');
    end
    ...
    Ribosome(i)     = x0(2);
    ppGpp(i)        = x0(3);
    [~,growthRate(i)] = Ecoli_GR_ODE_PMC(0,x0,kcat(i),hp);
    if (growthRate(i)<tol)
        indexCUT = i+1;
        break;
    end
