function [kcat,growthRate,Methionine,Ribosome,ppGpp] = run_nutrient_limitation(hp)
...
kcat                = 10.^[-4:0.05:4]/3600;  % turn over rate of metE
Methionine          = zeros(length(kcat),1);
Met_ox              = zeros(length(kcat),1);
Ribosome            = zeros(length(kcat),1);
MetE_SH             = zeros(length(kcat),1);
MetE_SSG            = zeros(length(kcat),1);
ppGpp               = zeros(length(kcat),1);
growthRate          = zeros(length(kcat),1);
...
x0 = [10,0,10,10,0,10];
tic;
[~,x,te] = ode15s(@Ecoli_GR_ODE_PMC,tspan,x0,options_ode15s_PMC,kcat(end),0,GSH,GSSG,Grx,hp);
if (isempty(te))
    x0 = x(end,:);
else
...
%   we run the simulation from high kcatient to low kcatient and stop it
%   when the growth rate becomes zero
indexCUT = 1;
for i=length(kcat):-1:1
    tic;
    [~,x,te] = ode15s(@Ecoli_GR_ODE_PMC,tspan,x0,options_ode15s_PMC,kcat(i),0,GSH,GSSG,Grx,hp);
    if (~isempty(te))
        error('Error: Oscillation Detected for the current parameter set!');
    end
    ...
    MetE_SSG(i)     = x0(5);
    ppGpp(i)        = x0(6);
    [~,growthRate(i)] = Ecoli_GR_ODE_PMC(0,x0,kcat(i),0,GSH,GSSG,Grx,hp);
    if (growthRate(i)<tol)
        indexCUT = i+1;
        break;
    end
...
plot(growthRate(indexCUT:end),Ribosome(indexCUT:end)*hp.Lr./hp.Pt/1.6,'k-','LineWidth',LW);
axis square;
box on;
...
plot(growthRate(indexCUT:end),Methionine(indexCUT:end),'k-','LineWidth',LW);
axis square;
box on;
set(gca,'YScale','log');
...
% % Methionine_RR1970            = [168,231,41,41,6,10,9,45,7,638,171,3,8];  %   uM
% % plot(growthrate_RR1970,Methionine_RR1970,'k.','MarkerSize',MS,'MarkerEdgeColor',ColorPalette(1,:));
% % plot([growthrate_RR1970-0.05,growthrate_RR1970+0.05],[median(Methionine_RR1970),median(Methionine_RR1970)],'k-','Color',ColorPalette(1,:),'LineWidth',LW);
...
% Methionine_IAK2016_slow      = [0.027,0.002,0.062,0.008,0.009,0.024,0.012,...
%     0.057,0.008,0.065,0.011,0.185]*1000; %   uM
% plot(growthrate_IAK2016_slow,Methionine_IAK2016_slow,'k.','MarkerSize',MS,'MarkerEdgeColor',ColorPalette(2,:));
% plot([growthrate_IAK2016_slow-0.05,growthrate_IAK2016_slow+0.05],[median(Methionine_IAK2016_slow),median(Methionine_IAK2016_slow)],'k-','Color',ColorPalette(2,:),'LineWidth',LW);
% 
% growthrate_IAK2016_fast     = 1.38;     %   1/h
% Methionine_IAK2016_fast      = [1.21,0.32,0.21,0.36,0.13,0.29,0.25,0.27,...
%     4.00,0.34,0.77,0.21,3.00]*1000;  %   uM
% plot(growthrate_IAK2016_fast,Methionine_IAK2016_fast,'k.','MarkerSize',MS,'MarkerEdgeColor',ColorPalette(2,:));
% plot([growthrate_IAK2016_fast-0.05,growthrate_IAK2016_fast+0.05],[median(Methionine_IAK2016_fast),median(Methionine_IAK2016_fast)],'k-','Color',ColorPalette(2,:),'LineWidth',LW);
...
% Methionine_BDB2009_slow      = [8.79e+2,5.40e+2,7.35e+3,4.48e+4,3.06e+3,...
%     9.75e+1,3.41e+2/2,5.54e+2,6.59e+1,2.74e+1,...
%     3.61e+2,9.55e+1,2.05e+1,5.22e+1,1.07e+3];    %   uM
% plot(growthrate_BDB2009_slow,Methionine_BDB2009_slow,'k.','MarkerSize',MS,'MarkerEdgeColor',ColorPalette(3,:));
% plot([growthrate_BDB2009_slow-0.05,growthrate_BDB2009_slow+0.05],[median(Methionine_BDB2009_slow),median(Methionine_BDB2009_slow)],'k-','Color',ColorPalette(3,:),'LineWidth',LW);
...
% Methionine_BDB2009_interm    = [1.77e+3,9.70e+2,9.30e+3,1.49e+5,4.95e+3,...
%     1.75e+2,4.38e+2/2,7.62e+2,1.29e+2,4.21e+1,...
%     4.51e+2,1.50e+2,2.36e+1,8.74e+1,2.29e+3];    %   uM
% plot(growthrate_BDB2009_interm,Methionine_BDB2009_interm,'k.','MarkerSize',MS,'MarkerEdgeColor',ColorPalette(3,:));
% plot(growthrate_BDB2009_interm,Met,'ko','MarkerSize',MS,'MarkerEdgeColor',ColorPalette(3,:));
% plot([growthrate_BDB2009_interm-0.05,growthrate_BDB2009_interm+0.05],[median(Methionine_BDB2009_interm),median(Methionine_BDB2009_interm)],'k-','Color',ColorPalette(3,:),'LineWidth',LW);
...
% Methionine_BDB2009_fast      = [2.55e+3,5.69e+2,5.11e+2,4.23e+3,9.60e+4,...
%     3.81e+3,6.76e+1,3.03e+2/2,4.05e+2,1.45e+2,...
%     1.82e+1,3.85e+2,6.80e+1,1.79e+2,1.21e+1,...
%     2.89e+1,4.02e+3];    %   uM
% plot(growthrate_BDB2009_fast,Methionine_BDB2009_fast,'k.','MarkerSize',MS,'MarkerEdgeColor',ColorPalette(3,:));
% plot(growthrate_BDB2009_fast,Met,'ko','MarkerSize',MS,'MarkerEdgeColor',ColorPalette(3,:));
% plot([growthrate_BDB2009_fast-0.05,growthrate_BDB2009_fast+0.05],[median(Methionine_BDB2009_fast),median(Methionine_BDB2009_fast)],'k-','Color',ColorPalette(3,:),'LineWidth',LW);
