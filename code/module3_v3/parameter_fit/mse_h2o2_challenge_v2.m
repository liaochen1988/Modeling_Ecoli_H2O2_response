function [dy, tspan, H2O2_output] = mse_h2o2_challenge_v2(p, vi_katG, Ki_katG, vi_ahpCF, Ki_ahpCF)
...
[~,x_wt] = ode15s(@h2o2_model, [0,1e10], x0, options, 0, false, alpha_max_katG, alpha_max_ahp, alpha_max_grx, vi_katG, Ki_katG, vi_ahpCF, Ki_ahpCF);
IC_wt = x_wt(end,:);

x0 = [0,0,0,0,0,0,InitN];
[~,x_katG] = ode15s(@h2o2_model, [0,1e10], x0, options, 0, true, alpha_max_katG, alpha_max_ahp, alpha_max_grx, vi_katG, Ki_katG, vi_ahpCF, Ki_ahpCF);
IC_katG = x_katG(end,:);
...
for i=1:length(H2O2_input)
    ...
    x0 = IC_wt;
    x0(1) = H2O2_IV(i,1);
    [~,x_wt] = ode15s(@h2o2_model, tspan, x0, options, 1, false, alpha_max_katG, alpha_max_ahp, alpha_max_grx, vi_katG, Ki_katG, vi_ahpCF, Ki_ahpCF);
    H2O2_output(i,1,:) = x_wt(:,1);
    
    % katG mutant
    x0 = IC_katG;
    x0(1) = H2O2_IV(i,2);
    [~,x_katG] = ode15s(@h2o2_model, tspan, x0, options, 1, true, alpha_max_katG, alpha_max_ahp, alpha_max_grx, vi_katG, Ki_katG, vi_ahpCF, Ki_ahpCF);
