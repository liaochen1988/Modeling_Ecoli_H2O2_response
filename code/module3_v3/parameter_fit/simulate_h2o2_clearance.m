...
for i=1:length(H2O2_input)
    ...
    % katG mutant
    x0 = [0,0,0,0,0,0,InitN];
    [~,x_wt] = ode15s(@h2o2_model, [0,1e10], x0, options, 0, true, alpha_max_katG, alpha_max_ahpCF, alpha_max_grx, vi_katG, Ki_katG, vi_ahpCF, Ki_ahpCF);
    x0 = x_wt(end,:);
    x0(1) = H2O2_IV(i,2);
    [~,x_wt] = ode15s(@h2o2_model, tspan, x0, options, 1, true, alpha_max_katG, alpha_max_ahpCF, alpha_max_grx, vi_katG, Ki_katG, vi_ahpCF, Ki_ahpCF);
    H2O2_output(i,2,:) = x_wt(:,1);
end
