...
PA = P*A/1e3; % L/s
k_met = 4.5e-20;
k_cat = 4.188e6*3.62e-24/(1.93/3600); % L/s
k_ahp = 1.19e-22/(1.93/3600)*52.4; % mol/s
Km_ahp = 1.2e-6; % M
n_ahp = 1;
...
lb = [-Inf];
ub = [0];
[p_opt,~,~,exitflag] = lsqnonlin(@mseOxyR_v3,p0,lb,ub,options,k_met,PA,k_cat,k_ahp,n_ahp,hill_opt_invitro,h2o2_invivo,oxyr_ox_invivo);
assert(exitflag>0);
Km_ahp = 10.^(p_opt(1));
