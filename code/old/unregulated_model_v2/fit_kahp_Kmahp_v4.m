...
h2o2_ahp_invivo = [0,0.585,0.875,1.368,4.942,4.967,5.007,9.961,15.098,15.023,27.982,39.990,39.976]/1e6; %M
Jahp_invivo = [0.024,1.398,2.176,3.023,4.085,5.055,9.491,13.882,14.477,16.730,15.500,16.667,17.704]; % uM/min*OD
...
lb = [-Inf,-Inf];
ub = [0,0];
[p_opt,~,~,exitflag] = lsqnonlin(@mseOxyR_v4,p0,lb,ub,options,k_met,PA,k_cat,n_ahp,hill_opt_invitro,...
                                 h2o2_invivo,oxyr_ox_invivo,h2o2_ahp_invivo,Jahp_invivo);
assert(exitflag>0);
k_ahp = 10.^(p_opt(1));
Km_ahp = 10.^(p_opt(2));
...
%% Predict H2O2 decomposition rate
Hout = 10.^[-9:0.001:-4]; % M
Hin = zeros(size(Hout));
options = optimset('Display','off','TolX',1e-12); % show iterations
for i=length(Hout):-1:1
    ...
    [Hin(i),fval,exitflag] = fzero(@calcHin,x0,options,0,Hout(i),PA,0,k_ahp,Km_ahp,n_ahp);
    while Hin(i)<0
        x0 = 10.^(-9+rand*5);
        [Hin(i),~,exitflag] = fzero(@calcHin,x0,options,0,Hout(i),PA,0,k_ahp,Km_ahp,n_ahp);
    end
    assert(exitflag>0);
end
Jahp = k_ahp.*Hin./(Hin+Km_ahp);
...
subplot(1,2,2);
hold on;
plot(Hout*1e6,Jahp/max(Jahp),'k-');
plot(h2o2_ahp_invivo*1e6,Jahp_invivo/max(Jahp_invivo),'ko');
axis square;
box on;