...
%% fit oxyR oxidation rate at various H2O2 concentration
...
%% fit oxidized OxyR fraction at different GSH/GSSG concentration
options = optimoptions('lsqnonlin','Display','iter','TolX',1e-6,'TolFun',1e-6,'MaxFunEval',Inf);
p0 = [0]; % Keq
lb = [-Inf];
ub = [Inf];
...
%% fit oxidized OxyR fraction at 30s when H2O2 concentration is varied
options = optimoptions('lsqnonlin','Display','iter','TolX',1e-6,'TolFun',1e-6,'MaxFunEval',Inf);
p0 = [11]; % k_oxyr_ox1
lb = [-Inf];
ub = [Inf];
[sol,~,~,exitflag] = lsqnonlin(@mse_vary_h2o2_foxyrox_dyn,p0,lb,ub,options,xdata_h2o2_foxyrox,ydata_h2o2_foxyrox,n_oxyr,Km_oxyr,k_oxyr_ox,Keq);
assert(exitflag>0);
...
yfit = k_oxyr_ox1./(k_oxyr_ox1+k_oxyr_red1*xfit.^4);
